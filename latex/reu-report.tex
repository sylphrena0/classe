...
\begin{figure*}[t] %star makes figure and caption span both columns
   %star environment floats are placed on the top of the next page or on dedicated float pages in revtex, no easy way around it
   \centering
...
\end{figure*}% fig:results-uncertainty

\clearpage

\subsection{Feature Selection} %abbreviate feature names for correlation plot, maybe use feature importance table
...
\begin{table}[!h]
   ...
   Mean Absolute Sim. Packing Efficiency           & 20.98\%               \\
   Mean Unfilled Valence $e^-$                     & 8.48\%                \\
   $L^0$ Norm Stoichiometric Attributes            & 5.61\%                \\
   Mean Space Group Number                         & 5.07\%                \\
   Mean Melting Temperature of Elements            & 3.92\%                \\
   Weighted Fraction of P Valence $e^-$            & 3.73\%                \\
   Mean Electronegativity of Elements              & 3.57\%                \\
   $L^2$ Norm Stoichiometric Attributes            & 2.78\%                \\ \hline
   \multicolumn{2}{|c|}{\textbf{Random Forest}}                            \\ \hline
   \textbf{Feature}                                & \textbf{Importance}   \\
   Mean Absolute Sim. Packing Efficiency           & 49.90\%               \\
   Mean Unfilled Valence $e^-$                     & 8.14\%                \\
   Weighted Fraction of D Valence $e^-$            & 6.21\%                \\
   $L^0$ Norm Stoichiometric Attributes            & 4.32\%                \\
   $L^2$ Norm Stoichiometric Attributes            & 1.90\%                \\
   Mean DFT Volume of Elemental Solids             & 1.88\%                \\
   Mean Unfilled P Orbital Valence $e^-$           & 1.81\%                \\
   Dist. from 1 Cluster $\lvert APE\rvert<0.010$\footnote{alloy composition and the cluster that has a packing efficiency below 0.010}    & 1.49\%                \\ \hline
   \end{tabular}%
   }
...
\end{table}% tab:feature-importance
...
\end{figure}% fig:feature-correlations

\section{Conclusions} %add discussion of Active learning bayesian optimization with prediction and uncertainty 
...
\end{figure*}% fig:selected-features
...
\end{document} %add option to export csv with prediction vs actual